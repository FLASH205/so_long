...
/*   Updated: 2025/02/04 10:20:17 by ybahmaz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	so_long(char *name_map)
{
	...
	if (!data->mlx_ptr)
		return (free(data), 0);
	if (map.width * SIZE > 3200 || map.height * SIZE > 1720)
		return (write(2, "Error\nMap is too big\n", 21), mlx_destroy_display(data->mlx_ptr),
			free(data->mlx_ptr), free(data), 0);
	data->window = mlx_new_window(data->mlx_ptr,
			map.width * SIZE, map.height * SIZE, "./so_long");
	if (!data->window)
